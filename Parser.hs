...
happyExpList = Happy_Data_Array.listArray (0,70) ([32880,128,0,0,239,0,0,448,61442,28718,32896,1027,8220,224,1793,14344,49216,513,0,0,0,32768,28,224,1824,14592,448,61442,28750,128,956,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","Exp","true","false","num","'+'","'-'","'*'","and","or","\"==\"","'>'","'<'","if","then","else","%eof"]
        bit_start = st * 19
        bit_end = (st + 1) * 19
...
action_25 (9) = happyShift action_9
